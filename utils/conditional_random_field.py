...
def get_o_tag_index(index_to_token_vocab: Dict[int, str]) -> int:
    for k in index_to_token_vocab:
        if index_to_token_vocab[k] == "O":
            return k
...
class CRF(torch.nn.Module):
    def __init__(
        self, num_tags: int, input_dim: int, label_encoding: str, index_to_token_vocab: Dict[int, str],
        include_start_end_transitions: bool = True, top_k: int = 1, ignore_loss_on_o_tags: bool = False,
        reduction: str = "sum"
    ) -> None:
        super(CRF, self).__init__()

        self.tag_projection_layer = torch.nn.Linear(input_dim, num_tags)

        constraints = allowed_transitions(label_encoding, index_to_token_vocab)
        self.crf = ConditionalRandomField(num_tags, constraints, include_start_end_transitions, reduction)

        self.top_k = top_k
        self.ignore_loss_on_o_tags = ignore_loss_on_o_tags
        self.o_tag_index = get_o_tag_index(index_to_token_vocab)

    def forward(
        self, logits: torch.Tensor, mask: torch.Tensor, gold_tags: torch.Tensor = None
    ) -> Tuple[torch.Tensor, torch.Tensor, Union[List[VITERBI_DECODING], List[List[VITERBI_DECODING]]]]:
        emissions = self.tag_projection_layer(logits)
        best_paths = self.crf.viterbi_tags(emissions, mask, top_k=self.top_k)

        # Just get the top tags and ignore the scores.
        predicted_tags = cast(List[List[int]], [x[0][0] for x in best_paths])
        max_len = max([len(pred_tags) for pred_tags in predicted_tags])
        predicted_tags = [pred_tags + [self.o_tag_index for _ in range((max_len - len(pred_tags)))] for pred_tags in predicted_tags]
        predicted_tags = torch.tensor(predicted_tags, dtype=torch.long, device=logits.device)

        if gold_tags is not None:
            if self.ignore_loss_on_o_tags:
                crf_mask = mask & (gold_tags != self.o_tag_index)
            else:
                crf_mask = mask
            # Add negative log-likelihood as loss
            loss = -self.crf(emissions, gold_tags, crf_mask)
        else:
            loss = torch.tensor(0.0, device=logits.device)
        
        return loss, predicted_tags, best_paths
